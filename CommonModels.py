...
class ResNet12(torch.nn.Module):
    def __init__(self, dim_output: typing.Optional[int] = None, bn_affine: bool = False, **kwargs) -> None:
        """Initialize an instance of ResNet-12
    ...
    def modified_resnet12(self) -> torch.nn.Module:
        """Create an instance of ResNet-12 with the batch-norm layer modified
        """
        # initialize a ResNet-12 instance
        net = torchvision.models.resnet._resnet(arch="resnet12", block=torchvision.models.resnet.BasicBlock, layers=[2, 1, 1, 1], pretrained=False, progress=False)
...
class WRN28_10(nn.Module):
    """A modified version of WRN-28-10 for meta-learning"""
    def __init__(self, dim_output: typing.Optional[int] = None, bn_affine: bool = False, **kwargs) -> None:
        """
        Args:
            dim_output: the number of classes at the output. If None,
        ...
        self.net = self.modified_wrn28_10()

    def modified_wrn28_10(self):
        """
        """
        def wide_basic_block(in_planes, planes, dropout_rate, stride=1):
            layers = []
            layers.append(nn.Conv2d(in_planes, planes, kernel_size=3, padding=1, bias=False))
            layers.append(nn.BatchNorm2d(planes, momentum=1, track_running_stats=False, affine=self.bn_affine))
            layers.append(nn.ReLU(inplace=True))
            layers.append(nn.Dropout(p=dropout_rate))
            layers.append(nn.Conv2d(planes, planes, kernel_size=3, padding=1, stride=stride, bias=False))
            layers.append(nn.BatchNorm2d(planes, momentum=1, track_running_stats=False, affine=self.bn_affine))
            
            shortcut = []
            if stride != 1 or in_planes != planes:
                shortcut.append(nn.Conv2d(in_planes, planes, kernel_size=1, stride=stride, bias=False))
                shortcut.append(nn.BatchNorm2d(planes, momentum=1, track_running_stats=False, affine=self.bn_affine))
        ...
        class WideResNet(nn.Module):
            def __init__(self, depth, widen_factor, dropout_rate, num_classes):
                ...
                self.layer1 = self._wide_layer(16 * widen_factor, num_blocks, dropout_rate, stride=1)
                self.layer2 = self._wide_layer(32 * widen_factor, num_blocks, dropout_rate, stride=2)
                self.layer3 = self._wide_layer(64 * widen_factor, num_blocks, dropout_rate, stride=2)
                self.bn1 = nn.BatchNorm2d(64 * widen_factor, momentum=1, track_running_stats=False, affine=self.bn_affine)
                self.linear = nn.LazyLinear(out_features=num_classes if num_classes is not None else 0)
            ...
            def _wide_layer(self, planes, num_blocks, dropout_rate, stride):
                ...
                for stride in strides:
                    block, shortcut = wide_basic_block(self.in_planes, planes, dropout_rate, stride)
                    layers.append(nn.Sequential(block, shortcut))
                    self.in_planes = planes

                return nn.Sequential(*layers)

            def forward(self, x):
                out = torch.nn.functional.relu(self.bn1(self.layer3(self.layer2(self.layer1(self.conv1(x))))))
                out = torch.nn.functional.avg_pool2d(out, 8)
                out = out.view(out.size(0), -1)
                out = self.linear(out)
                return out

        net = WideResNet(depth=28, widen_factor=10, dropout_rate=self.dropout_prob, num_classes=self.dim_output)
        return net
